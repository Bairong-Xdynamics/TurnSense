...
def ort_providers(prefer_gpu: bool = True):
    """
    Return (providers, provider_options) for onnxruntime.
    Prefer CUDA/ROCm when available; otherwise fallback to CPU.
    """
    try:
    ...
    if prefer_gpu and ("ROCMExecutionProvider" in avail):
...
# ---------------------------
# Labels (✅ interruption -> dismissal; ✅ wait -> dismissal)
# ---------------------------
LABELS_4 = {"complete", "incomplete", "dismissal", "backchannel"}
...
TAG_ALIASES = {
    ...
    # backward compatibility: interruption -> dismissal
    "interruption": "dismissal",
    "interrupt": "dismissal",
...
def extract_gt_label_from_last_assistant(messages: List[Dict[str, Any]]) -> Optional[str]:
    """
    GT: 从最后一条 assistant.content 解析 <|Completion|> / <|Backchannel|> / <|Dismissal|> / <|Wait|> / <|Incomplete|> ...
    """
    if not messages:
        return None
    last = messages[-1]
...
def load_dataset(path: str, default_lang: Optional[str] = None, max_history: int = 6) -> List[Dict[str, Any]]:
    ...
    Args:
        path: 数据路径
        default_lang: 如果指定（"zh"/"en"），则统一覆盖注入
        max_history: 上下文长度（给带 context 的模型用）
    """
    p = Path(path)
    if not p.exists():
    ...
    src_stem = p.stem  # ✅ 用于“按文件拆分 wait/dismissal 统计”
...
class Confusion4:
    ...
    def update(self, pred: str, gt: str, lang: str, src: Optional[str] = None):
        self.total += 1
        ok = (pred == gt)
        if ok:
            self.correct += 1
        ...
        # ---- binary accounting: pos=complete, neg=non-complete ----
...
class BaseTurnModel:
    ...
    supported_labels: set = {"complete", "incomplete"}  # default binary
